...
public class PortfolioReport {
	...
	private static void portfolioSummaryReport(List<Portfolio> portfolio, List<Asset> assetList,
		...
		for (Portfolio x : portfolio) {
			...
			returnTotal += Asset.getTotalRateOfReturn(tempAssets, x);
			totalTotal += Asset.getTotalPortfolioValue(tempAssets, x);
			...
			System.out.printf("%-10s %-20s %-20s %-5s %-10.2f %-5s %-10.2f %-5s %-10.4f %-5s %-10.2f %-5s %-10.2f\n",
					...
					broker.getCommission(tempAssets, x), "$", Asset.getWeightedRisk(tempAssets, x), "$",
					Asset.getTotalRateOfReturn(tempAssets, x), "$", Asset.getTotalPortfolioValue(tempAssets, x), "$",
	...
	public static void main(String args[]) {
		...
		PortfolioReport.portfolioSummaryReport(portfolio, assetList, personList);
		PortfolioReport.portfolioDetails(portfolio, assetList, personList);
